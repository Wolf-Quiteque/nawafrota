...
/** Parses a form field that may be '', null, or a comma-decimal ("150,5"). */
export function parseAmount(value) {
  if (value === null || value === undefined) return null;
  if (typeof value === 'number') return Number.isFinite(value) ? value : null;
  const trimmed = String(value).trim();
  if (!trimmed) return null;
  // Angolan keyboards produce ',' for the decimal mark.
 ...
 */
export function deriveFuelAmounts(values, changed) {
  ...
  if (changed === 'litres') {
    ...
    if (litres !== null && price !== null) next.totalCost = round2(litres * price);
    else if (canDivide(price) && total !== null) next.litres = round2(total / price);
  } else if (changed === 'totalCost') {
    if (canDivide(litres) && total !== null) next.pricePerLitre = round2(total / litres);
    else if (canDivide(price) && total !== null) next.litres = round2(total / price);
  }
...
export function validateFuelEntry(input) {
  ...
  if (litres === null) errors.litres = 'Indique os litros.';
