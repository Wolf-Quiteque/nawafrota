...
export async function maxSoldSeatForBus(busId) {
  const supabase = createSupabaseAdminClient();
  // Keep the join in Postgres. The previous version first downloaded every
  // future trip id and then sent the whole list back in a second URL. A busy
  // bus can have 150+ route rows, making the edit screen slow and vulnerable
  // to proxy URL/header limits.
  const { data, error } = await supabase
    .from('tickets')
    .select('seat_number, trip:trips!inner(bus_id, departure_time)')
    .eq('trip.bus_id', busId)
    .gt('trip.departure_time', new Date().toISOString())
